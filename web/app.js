...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function renderDetailView(data) {
        ...
        if (data.last_updated_date && data.last_updated_date.value) {
            formattedLastUpdatedDate = new Date(
                data.last_updated_date.value
        ...
        if (data.evaluation_date && data.evaluation_date.value) {
            formattedEvaluationDate = new Date(
                data.evaluation_date.value
            ).toLocaleDateString('en-US', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
    ...
    function fetchProductAreas(language) {
        return apiFetch(
            `/product-areas?language=${encodeURIComponent(language)}`
        );
    }
    function fetchRegionTags(language, productArea) {
        return apiFetch(
            `/region-tags?language=${encodeURIComponent(language)}&product_name=${encodeURIComponent(productArea)}`
        );
    }
    function fetchEvaluationDetails(language, productArea, regionTag) {
        return apiFetch(
